...
function ProfileSiswa() {
  ...
  const [tempData, setTempData] = useState(null); // Untuk menyimpan data untuk diedit
  ...
  useEffect(() => {
    ...
    if (!storedId) {
      ...
      fetch(`http://localhost:3000/api/users/${storedId}`) // Endpoint API untuk mendapatkan data siswa
        .then((response) => {
        ...
        .then(data => {
          ...
          setTempData(userWithoutPassword);
        })
        .catch(err => {
  ...
  const handleDelete = async () => {
    ...
    if (confirmed) {
      ...
      // Mengirim permintaan untuk menghapus siswa berdasarkan ID
  ...
  const handleSubmit = async (e) => {
    ...
    if (response.ok) {
      ...
      setTempData(updatedData); // Update temporary data
